...
**Phase 9 candidate generation complete to first-pass depth. Phase-10 evidence has now produced eight provisional selections for the current experimental generation. No whole architecture family has been selected.**
...
| DL-006 | belief ambiguity | one best state / multiple hypotheses / adaptive hybrid | **provisional: consequence-sensitive hypothesis plurality** — preserve multiple plausible states while ambiguity × consequence justifies the extra state/coordination cost; collapse toward a single working belief as evidence becomes decisive or wrong commitment becomes cheap | E06 noisy four-world family: multi-hypothesis expected-utility decisions strongly reduce wrong commitments under weak evidence and converge toward single-belief performance under clear evidence; E23B same-intervention causal ablation shows preserving unresolved theories dominates forced commitment as false-theory cost rises, while forced single commitment wins when false claims are cheap | current state is always one fact set; maintaining alternatives is always superior; uncertainty should never delay commitment | medium-high for the adaptive plurality principle; hypothesis representation/generation/pruning open | show a single-belief system consistently matches or exceeds lifetime utility under ambiguity/consequence shifts after equal state cost, or show plurality-management overhead grows faster than avoided commitment loss in realistic hypothesis spaces |
| DL-007 | information acquisition | passive / fixed query / value-driven active acquisition | **provisional: value-driven active evidence acquisition with non-myopic option value** — buy observations/interventions only when expected downstream or epistemic value exceeds interaction cost/risk; account for complementary sequences where one observation is valuable mainly because it enables another | E07 noisy-probe family: bounded lookahead adapts from full querying to selective querying to zero querying as probe cost rises and can outperform one-step VOI when probes are complementary; E23B causal-science family: active interventions identify worlds passive teacher evidence cannot distinguish and stop unresolved when experiments become too expensive | perception is only incoming data; more observation is always better; one-step information gain is sufficient; fixed experiment schedules are efficient across prices | medium-high for the principle; scalable learned VOI/experiment design open | if fixed/passive acquisition matches adaptive lifetime utility under changing informativeness/cost, or if planning overhead/VOI miscalibration overwhelms gains on larger partially observed systems |
...
| DL-023 | epistemic frontier policy | imitate/inherit only / unconstrained novelty / search-and-experiment with staged independent verification | **provisional: verified frontier expansion** — bootstrap/teacher knowledge may guide the proposal prior without defining the epistemic ceiling; novel claims remain hypotheses until sufficiently independent evidence verifies the relevant property, and unresolved/rejected hypotheses retain explicit status | E23 constructive family: diverse stepping-stone search crosses a deceptive teacher frontier 30/30 with a correct evaluator, while the same search exploits a defective evaluator 29/30 unless hidden verification gates consolidation; E23B empirical family: active interventions identify hidden theories unavailable from passive teacher evidence and decline to assert when experiments are not worth their cost | supervisor agreement is the permanent truth criterion; novelty is evidence; a visible evaluator score is equivalent to knowledge; every research question must be resolved | high for separation of novelty/knowledge and staged independent evidence; open for truly autonomous frontier research | externally verified open-domain trials show independent staging adds no reliability, or reliable discovery cannot exceed supervisor/corpus frontiers without hidden answer leakage/additional evidence unavailable to the claimed system |
...
### PS-006 — consequence-sensitive hypothesis plurality (from DL-006)

The system should not force one current world state when several materially different states remain plausible and the consequence of choosing the wrong one is high. Preserve alternatives while their expected decision/discovery value exceeds the cost of carrying them; collapse/prune as evidence resolves ambiguity or stakes fall.

The selected object is the **plurality rule**, not Bayesian enumeration or the current four-state toy representation.

### PS-007 — value-driven active evidence acquisition (from DL-007)

Observation and experiment are operations that consume resources and can change future decisions. Acquire evidence when its expected downstream or epistemic value exceeds cost/risk, including multi-step option value when observations are complementary. Stop acquiring when the remaining information is not worth its price.

The selected object is the acquisition policy principle, not the current exact two-step planner.

### PS-008 — verified epistemic frontier expansion (from DL-023)

Human/bootstrap knowledge is a starting prior and method library, not a permanent epistemic ceiling. The system may search or experiment beyond demonstrated answers, but novelty remains tentative. A candidate discovery becomes durable knowledge only through domain-appropriate evidence sufficiently independent from the proposal/optimization path, with verifier scope/provenance retained. Rejected and unresolved hypotheses remain distinct epistemic states.

This is a discovery-governance principle, not a claim that the current synthetic organism has produced new human knowledge.
...
- **Belief ambiguity:** E06 + E23B support PS-006: preserve alternatives conditionally rather than equating current state with one asserted fact set.
- **Information acquisition:** E07 + E23B support PS-007: sensing/experimentation is an allocatable operation with potentially non-myopic value.
- **Discovery:** E23 + E23B support PS-008: supervision need not define the ceiling, but search power must co-scale with independent evidence and epistemic staging.
...
No family is selected because PS-001 through PS-008 can all be implemented by multiple families.
