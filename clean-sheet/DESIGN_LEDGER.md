...
I22/I23 extend this through crash recovery: source-backed hot state can be selectively persisted/rematerialized instead of restoring every cached value.
...
Preparation cannot capture authority. Historical execution evidence may establish that an effect already happened after revocation, but every fresh/retry effect and recovered old-epoch event re-checks current permission.
...
I23 demonstrates this inside a lifecycle that simultaneously restores transient work, old events and external evidence.
...
Source-backed hot state can be rematerialized instead of blindly persisted. Delayed credit requires exact historical causal identity/version or source replay rather than positional restoration after structural change. `TransientStateRegistry` records only the semantic recovery metadata needed for those decisions.
...
crash recovery from semantic identity/provenance
        ↓
old-epoch event forwarding + authority refresh
        ↓
TransientStateRegistry
  hot state -> currentness / rematerialization
  credit -> exact causal identity / replay source
        ↓
